...
入力は１文字２打鍵で行う。一打目で行（あかさたな）を指定し、二打目で段（あいうえお）を指定する。あ行の入力も二打鍵で行うことに注意。
...
「ヴァ行」と「ヴ」は、わ行拗音拡張の同手、「ウィ行」は、わ行拗音拡張の異手、「ツァ行」と「トゥ」は、や行拗音拡張の異手（た行拗音拡張の一つ右）、「ヅァ」行と「ドゥ」はや行拗音拡張の同手で入力できる。
...
例：　ｊｐ　→　〜　（波ダッシュ。ただし、Windows環境では全角チルダが入力される）  
　　　ｊ／　→　～  （全角チルダ）  
　　　ｈｐ　→　―　 （和文ダッシュその１(U+2015)）     
　　　ｈ／　→　――　（和文倍角ダッシュその１(U+2015 ２つ)）    

Uから始まる同手の組み合わせにも若干の記号を配置している。
...
> 漢字の直接入力拡張を利用するとシフトキーを多用することになります。シフトキーは押しやすいキーではなく、ホームポジションも崩れてしまいますので、スペースバーにシフトキーを兼用させる（ http://semialt.hatenablog.com/entry/2014/04/26/022124 ）といいでしょう。
...
> 漢字や単語、文字列、短文の配置は実験的なものであり、変更されることがあります。逆に言えば、この定義にこだわる必要はありませんので、自分が使いやすい位置に頻繁に使う漢字を配置してしまいましょう。

改造等について
--------
日本語配列に著作権が生じるかは疑問なので、こまどり配列には特にライセンスをつけていません。使用・複製・改造等は自由に行なって下さい。

こまどり配列派生の日本語配列を公開するときは、なるべくこまどり配列の影響に触れてくれると嬉しいです。こまどり配列はつばめ配列とAZIKの派生配列なので、こまどり配列経由でつばめ配列やAZIKの特徴を受け継いでいる場合はつばめ配列やAZIKの名前も挙げてくれると嬉しいです。
