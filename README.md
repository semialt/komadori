...
あ行とや行の場合は、濁音のかわりに、小文字が入力される。

例：　ｇｇ　→　ょ    
　　　ａｓ　→　ぃ

ま行（は行の一つ右）の同手は、ぱ行が入力される。
...
> 基本的な入力方法は、キーボードの中段のみで文字を入力できる「つばめ配列」( http://layout.kachoufuugetu.net/original/tsubame/ ）の考え方をほぼそのまま採用している。ただし、具体的な配置は異なる。
