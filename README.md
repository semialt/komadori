...
「ふぃ行」は、りゃ行の同手（ら行拗音拡張の同手）で入力することができる。
...
一打目をシフトして、大文字を入力することで、訓読みが該当する漢字を入力できる。
...
同じ訓読みの漢字が二つ登録されているときは、二打目をシフトすることで２つ目を入力できる。

例：　ｓＰ　→　回

１音では表現できない漢字等は、一打目と二打目を両方シフトし、複数の音の行をそれぞれとったキーを打つことで入力できる。
